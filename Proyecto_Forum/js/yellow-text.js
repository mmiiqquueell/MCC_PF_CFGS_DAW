...
;(function ( $, window, document, undefined ) {
    ...
    var pluginName = "YellowText",
        defaults = {
          ...
          cleanOnSubmit    : true,
          defaultFont      : "SMGP, Helvetica Neue, Helvetica, arial, sans-serief",
          defaultFontSize  : "1em",
    ...
    YellowText.prototype = {
      ...
      */
      render: function() {
        ...
        var iFrameCSS = "<style type=\"text/css\">body{padding:1%;}p{margin:0;}</style>";
      ...
      createButtons: function() {
        ...
        for( var i = 0; i < this.options.defaultActions.length; i++ ) {
          ...
          switch( this.options.defaultActions[i] ) {
            case "bold" :
              button = { content : "B", command : "bold", clase : "font-weight-bold" };
            break;
            case "underline" :
              button = { content : "U", command : "underline", clase : "subrayado" };
            break;
            case "italic" :
              button = { content : "I", command : "italic", clase : "font-italic" };
            break;
            case "strikethrough" :
              button = { content : "-S-", command : "strikethrough", clase : "tachado" };
            break;
            case "align-left" :
              button = { content : "left", command : "JustifyLeft", clase : "JustifyLeft" };
            break;
            case "align-center" :
              button = { content : "center", command : "JustifyCenter", clase : "JustifyCenter" };
            break;
            case "align-right" :
              button = { content : "right", command : "JustifyRight", clase : "JustifyRight" };
            break;
            case "unordered-list" :
              button = { content : "ul", command : "InsertUnorderedList", clase : "InsertUnorderedList" };
            break;
            case "ordered-list" :
              button = { content : "ol", command : "InsertOrderedList", clase : "InsertOrderedList" };
            break;
            case "image" :
              button = { content : "img", command : "image", clase : "image" };
            break;
            case "link" :
              button = { content : "link", command : "link", clase : "link" };
            break;
            default :
          ...
          $("<a />").addClass( button.clase ).text( button.content ).data( "command", button.command ).appendTo( this.buttons );
        }
      },
      ...
      */
      runCMD: function( cmd ) {
        ...
        if( cmd === "image" ) {
          ...
          var url = ( typeof image !== "undefined" && image.length > 0 ) ? image : prompt("Insertar imagen (ejemplo: http://www.google.com/image/avion.jpg): ");

          // Insert the image in the text editor
          return this.editor.contentWindow.document.execCommand( "InsertImage", false, url);
        } else if( cmd === "link" ) {
          var link = prompt("Insertar dirección URL (ejemplo: http://www.google.com): ");
